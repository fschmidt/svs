...
def disguise(macpwhash, pwhash, imagePath, dataPath, target, useHeader = False):
    print('################################################################')
    print('##########################DISGUISE##############################')
    print('################################################################')
    print("> disguising: " + dataPath + " in " + imagePath)
    ...
    if useHeader:
        ...
        data = header + macpwhash + data
    else:
        data = macpwhash + data

    bits = asBits(encrypt(pwhash, data))
...
def reveal(macpwhash, imagePath, useHeader = False):
    print('################################################################')
    print('###########################REVEAL###############################')
    print('################################################################')
    print("> revealing text from : " + imagePath)
    print("> using fast mode : " + str(useHeader))
    image = Image.open(imagePath)
    image_data = image.getdata()
    hbits = []
    ...
    if useHeader:
        length = readHeader(decrypt(pwhash, toString(hbits, 0, 4)))
    else:
        length = len(image_data)
    bits = []
    strLen = length + len(macpwhash) + 4
    for i in range(len(image_data)):
        if len(bits) >= strLen * 8:
            break
    ...
    if useHeader:
        return toString(bits, 0, strLen)
    else:
        return toString(bits, 0, len(bits))
...
def encrypt(pwhash, text):
    key = pwhash[:16]
    print("> encrypting text with key: " + key)
    result = crypt(key, text)
    result.encode('hex')
    return result
...
if __name__ == "__main__":
    ...
    parser.add_option("--h", action = "store_true", dest = "header", default = False)
    ...
    if len(args) == 1 :
        s = reveal(macpwhash, args[0], options.header)
        s = decrypt(pwhash, s)
        if options.header:
            s = s[4:]
        ...
        tmp = disguise(macpwhash, pwhash, args[1], args[0], args[1] + '.ste', options.header)
